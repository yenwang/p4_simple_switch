...
### key point for generating arp packet or not, /24 specify different subnet and if host are in different subnet then no arp packet is generated( instead dst mac as switch interface)
            host_ip = "10.0.%d.%d" % (sw_num, host_num)
	    ''' if host_num==4:
		host_ip = "10.0.%d.%d" % (host_num, host_num)
	    else:
		host_ip = "10.0.1.%d" % host_num '''
...
class ExerciseRunner:
    ...
    def program_hosts(self):
	    ...
	    print "host_name=%s, h=%s, h_iface=%s, link=%s, sw_iface=%s\n" % (host_name, h, h_iface, link,sw_iface)
	    gid = str(sw_iface)
	    gate_id = int(gid[1:2])
